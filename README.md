...
Профиль закрепляет текущий публичный IPv4 в `ServerAddresses`. После обновления
до `v1.0.28` скачайте профиль заново через пункт «Доступ к данным» и
переустановите его на устройстве; уже установленный профиль не обновляется сам.
...
закрыт. HTTPS не расшифровывается: nginx использует TLS SNI. Наружу нужны только
TCP 80 и TCP 443; порт 53 открывать не требуется.

## Устойчивость DNS

Устройство Apple с установленным DoH-профилем не откатывается на обычный DNS:
если сервер перестал отвечать, на устройстве нет DNS вообще — ни для одного
сайта. Поэтому конфигурация держит несколько независимых путей восстановления:
